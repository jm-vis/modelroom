"""Criterion 12 of `scripts/selftest.py`: a machine entered by hand, in a guided run of its own.

12. **A machine entered by hand.** One more guided run, isolated from the two of criteria 1 to 6:
...
ENTERED_ANSWERS = {
    ...
    "users": 1,  # one person, one request: the pool of 23 GiB holds for one KV cache (criterion 11's answer)
    "context": "S",
    "pull": False,
}
NAME = "(12) a machine entered by hand ranks on shared memory, every number computed"
...
def entered_problems(payload: dict, local: str) -> list[str]:
    """Criterion 12 on the document: this machine without a profile, and the one entered by hand."""
...
def entered_steps(work: Path, answers_toml: Callable[[dict], str], now: datetime) -> list[tuple]:
    """Criterion 12 as `(name, ok, detail)`, for `scripts/selftest.py` to report."""
