...
packager` for an owner class of `other`) -- the repositories themselves are no lines of the list
either. What the list has to say about itself is its **instruction line**
(`guided_models.hint_line`), under the list and behind the line of the keys: `latest: the
publisher's current release of its family · legacy: the publisher named a successor · fit from the size at 32k
context, exact after the fetch` -- with `fit unknown until this machine is measured` in place of the
last piece when this folder holds no measured machine, and always the context the fit was really
computed for. What nothing marked would do is no longer said there: `Enter` takes the row under the
pointer now.
